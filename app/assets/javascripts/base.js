...
$(function() {
	...
	$("a.show-replies").live("click", function(){ 
		...
		$(".sort-links a").live('click', function(){
			$.getScript(this.href);
			return false;
		});
		
		// $("#bet-title-link").live('click', function(){
		// 			$(this).parent().parent().find(".bet-body").slideToggle("normal");
		// 			return false;
		// 		});
		
		$(".view-all-bets a").live('click', function(){
			$.getScript(this.href);
			return false;
		});
...
function showWagers(){
	$("#bet-wagers-list").slideToggle("normal");
...
function bindCalendar(){
	if ($("#bet_end_date").length > 0){
		$("#bet_end_date").before("<span id='datepicker'></span>");
		$("#bet_end_date").hide();
		$("#datepicker").datepicker({
			dateFormat: 'yy-mm-dd',
			defaultDate: $("#bet_end_date").val().split(" ")[0],
